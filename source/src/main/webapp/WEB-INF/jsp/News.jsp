                ...
                <details class="news_request">
                    ...
                    <div class="accordion">
	                    <div class="details">
	                        ...
	                        <div>
	                        	<p class="day">依頼実行日時:<br>${item.date} &nbsp; ${item.time}</p><br>
	                        	<p class="address">住所:<br>${item.address} </p><br>
	                        </div>
	                   </div> 
                        <div class="price">
                            <c:if test="${not empty item.option1}"><p>・${item.option1}</p>&nbsp;<p>${item.cost1}</p></c:if>
                            <c:if test="${not empty item.option2}"><p>・${item.option2}</p>&nbsp;<p>${item.cost2}</p></c:if>
                            <c:if test="${not empty item.option3}"><p>・${item.option3}</p>&nbsp;<p>${item.cost3}</p></c:if>
                            <c:if test="${not empty item.option4}"><p>・${item.option4}</p>&nbsp;<p>${item.cost4}</p></c:if>
